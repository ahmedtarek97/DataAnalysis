...
def batting_average(info, batting_stats):
    ...
    atbats = float(batting_stats[info["atbats"]])
    if atbats >= MINIMUM_AB:
        return hits / atbats
    else:
        return 0
...
def slugging_percentage(info, batting_stats):
    ...
    homeruns = float(batting_stats[info["homeruns"]])
...
def filter_by_year(statistics, year, yearid):
    ...
    batting_by_year = [d for d in statistics if year == int(d[yearid])]
    return batting_by_year
